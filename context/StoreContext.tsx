...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [isFirebaseEnabled, setIsFirebaseEnabled] = useState(isFirebaseConfigured());



  // Setup real-time listeners for orders and tables
  useEffect(() => {
    ...
    const unsubTables = firestoreService.subscribeToTables((newTables) => {
      setTables(newTables);
    });
  ...
  const addOrder = (order: Order) => {
    ...
    let finalKitchenStatus = isCompleted ? OrderStatus.COMPLETED : (hasKitchenItems ? OrderStatus.PENDING : OrderStatus.NONE);
    let finalBarStatus = isCompleted ? OrderStatus.COMPLETED : (hasBarItems ? OrderStatus.PENDING : OrderStatus.NONE);

    // FAILSAFE: If neither station is selected and order is not complete, default to Kitchen
    if (!isCompleted && finalKitchenStatus === OrderStatus.NONE && finalBarStatus === OrderStatus.NONE) {
      finalKitchenStatus = OrderStatus.PENDING;
    }
    ...
    const newOrder: Order = {
      ...
      kitchenStatus: finalKitchenStatus,
      barStatus: finalBarStatus
    };
    ...
    if (newOrder.status === OrderStatus.COMPLETED && newOrder.customerId) {
      setCustomers(prev => prev.map(c => {
        if (c.id === newOrder.customerId) {
          ...
          const updatedCustomer = { ...c, points: newPoints, visitCount: c.visitCount + 1, tier: calculateTier(newPoints) };
          if (isFirebaseEnabled) firestoreService.saveCustomer(updatedCustomer);
        ...
        menuItem.recipe.forEach(recipeItem => {
          ...
          if (invIndex !== -1) {
            ...
            if (currentQty !== newQty) {
              ...
              newInventory[invIndex] = updatedItem;
              if (isFirebaseEnabled) firestoreService.saveInventoryItem(updatedItem);
            }
          }
        });
      }
  ...
  const updateOrderItemStatus = (orderId: string, itemIndex: number, status: OrderStatus) => {
    setOrders(prev => prev.map(o => {
      ...
      const updatedOrder = { ...o, items: newItems, kitchenStatus: newKitchenStatus, barStatus: newBarStatus, status: newGlobalStatus };
      if (isFirebaseEnabled) firestoreService.saveOrder(updatedOrder);
      return updatedOrder;
    }));
  };
  ...
  const updateOrderStationStatus = (orderId: string, station: 'kitchen' | 'bar', status: OrderStatus) => {
    setOrders(prev => {
      ...
      if (isFirebaseEnabled) firestoreService.saveOrder(newOrder);
  ...
  const updateOrderDiscount = (orderId: string, discount: number) => {
    setOrders(prev => prev.map(o => {
      if (o.id === orderId) {
        const updated = { ...o, discount: Math.min(discount, o.total) };
        if (isFirebaseEnabled) firestoreService.saveOrder(updated);
        return updated;
      }
      return o;
    }));
  ...
  const voidOrder = (orderId: string, reason: string) => {
    ...
    if (order.status === OrderStatus.COMPLETED && order.customerId) {
      setCustomers(prev => prev.map(c => {
        if (c.id === order.customerId) {
          ...
          const newPoints = Math.max(0, c.points - earned + redeemed);
          const updated = { ...c, points: newPoints, visitCount: Math.max(0, c.visitCount - 1), tier: calculateTier(newPoints) };
          if (isFirebaseEnabled) firestoreService.saveCustomer(updated);
          return updated;
        }
        return c;
      }));
    ...
    const voidedOrder = {
      ...
      voidReason: reason,
      items: order.items.map(i => ({ ...i, status: OrderStatus.CANCELLED }))
    };

    setOrders(prev => prev.map(o => o.id === orderId ? voidedOrder : o));
    if (isFirebaseEnabled) firestoreService.saveOrder(voidedOrder);
    ...
    setInventory(prev => prev.map(item => {
      if (item.id === updatedItem.id) {
        ...
        if (diff !== 0) {
          ...
          const updated = { ...updatedItem, logs: [log, ...item.logs] };
          if (isFirebaseEnabled) firestoreService.saveInventoryItem(updated);
          return updated;
        }
        if (isFirebaseEnabled) firestoreService.saveInventoryItem({ ...updatedItem, logs: item.logs });
  ...
  const addInventoryItem = (item: InventoryItem) => {
    ...
    const newItem = { ...item, logs: [log] };
    setInventory(prev => [...prev, newItem]);
    if (isFirebaseEnabled) firestoreService.saveInventoryItem(newItem);
  };

  const deleteInventoryItem = (id: string) => {
    setInventory(prev => prev.filter(i => i.id !== id));
    if (isFirebaseEnabled) firestoreService.deleteInventoryItemFromDb(id);
  };

  const updateMenuItem = (item: MenuItem) => {
    setMenu(prev => prev.map(m => m.id === item.id ? item : m));
    if (isFirebaseEnabled) firestoreService.saveMenuItem(item);
  };
  ...
  const deleteMenuItem = (id: string) => {
    ...
    if (isFirebaseEnabled) firestoreService.deleteMenuItemFromDb(id);
  };

  const addCategory = (category: Category) => {
    setCategories(prev => [...prev, category]);
    if (isFirebaseEnabled) firestoreService.saveCategory(category);
  };
  ...
  const deleteCategory = (id: string) => {
    ...
    if (isFirebaseEnabled) firestoreService.deleteCategoryFromDb(id);
  };

  const addTable = (table: Table) => {
    setTables(prev => [...prev, table]);
    if (isFirebaseEnabled) firestoreService.saveTable(table);
  };
  ...
  const deleteTable = (id: string) => {
    ...
    if (isFirebaseEnabled) firestoreService.deleteTableFromDb(id);
  };

  const updateTableStatus = (id: string, status: TableStatus) => {
    setTables(prev => prev.map(t => {
      if (t.id === id) {
        const updated = status === TableStatus.AVAILABLE
          ? { ...t, status, mergedWith: [], isCallingStaff: false }
          : { ...t, status };
        if (isFirebaseEnabled) firestoreService.saveTable(updated);
        return updated;
      }
      return t;
    }));
  ...
  const toggleTableCall = (tableId: string, status: boolean) => {
    setTables(prev => prev.map(t => {
      if (t.id === tableId) {
        const updated = { ...t, isCallingStaff: status };
        if (isFirebaseEnabled) firestoreService.saveTable(updated);
        return updated;
      }
      return t;
  ...
  const addZone = (zone: Zone) => {
    ...
    if (isFirebaseEnabled) firestoreService.saveZone(zone);
  };
  ...
  const deleteZone = (id: string) => {
    ...
    if (isFirebaseEnabled) firestoreService.deleteZoneFromDb(id);
  };

  const updateSettings = (newSettings: SystemSettings) => {
    setSettings(newSettings);
    if (isFirebaseEnabled) firestoreService.saveSettings(newSettings);
    ...
    setOrders(prev => prev.map(o => {
      if (o.id === orderId) {
        ...
        if (newItems.length === 0) {
          ...
          const cancelled = { ...o, items: [], total: 0, status: OrderStatus.CANCELLED, kitchenStatus: OrderStatus.CANCELLED, barStatus: OrderStatus.CANCELLED, voidReason: 'All items removed' };
          if (isFirebaseEnabled) firestoreService.saveOrder(cancelled);
          return cancelled;
        }
        ...
        const updated = { ...o, items: newItems, total: newTotal };
        if (isFirebaseEnabled) firestoreService.saveOrder(updated);
        return updated;
      }
      return o;
  ...
  const addBooking = (booking: Booking) => {
    ...
    if (isFirebaseEnabled) firestoreService.saveBooking(booking);
    if (booking.status === 'Confirmed' && booking.tableId) updateTableStatus(booking.tableId, TableStatus.RESERVED);
  };

  const updateBooking = (updatedBooking: Booking) => {
    setBookings(prev => prev.map(b => b.id === updatedBooking.id ? updatedBooking : b));
    if (isFirebaseEnabled) firestoreService.saveBooking(updatedBooking);
  ...
  const deleteBooking = (id: string) => {
    ...
    if (isFirebaseEnabled) firestoreService.deleteBookingFromDb(id);
  };
  ...
  const mergeTables = (masterId: string, slaveId: string) => {
    setTables(prev => {
      const updated = prev.map(t => {
        if (t.id === masterId) {
          const merged = { ...t, mergedWith: [...(t.mergedWith || []), slaveId] };
          if (isFirebaseEnabled) firestoreService.saveTable(merged);
          return merged;
        }
        if (t.id === slaveId) {
          const slave = { ...t, status: TableStatus.OCCUPIED };
          if (isFirebaseEnabled) firestoreService.saveTable(slave);
          return slave;
        }
        return t;
      });
  ...
  const unmergeTables = (masterId: string, slaveId: string) => {
    setTables(prev => prev.map(t => {
      if (t.id === masterId) {
        const updated = { ...t, mergedWith: (t.mergedWith || []).filter(id => id !== slaveId) };
        if (isFirebaseEnabled) firestoreService.saveTable(updated);
        return updated;
      }
      if (t.id === slaveId) {
        const hasOrders = orders.some(o => o.tableId === slaveId && ![OrderStatus.COMPLETED, OrderStatus.CANCELLED].includes(o.status));
        const updated = { ...t, status: hasOrders ? TableStatus.OCCUPIED : TableStatus.AVAILABLE };
        if (isFirebaseEnabled) firestoreService.saveTable(updated);
        return updated;
      }
      return t;
  ...
  const transferTable = (fromId: string, toId: string) => {
    setOrders(prev => prev.map(o => {
      if (o.tableId === fromId && ![OrderStatus.COMPLETED, OrderStatus.CANCELLED].includes(o.status)) {
        const updated = { ...o, tableId: toId };
        if (isFirebaseEnabled) firestoreService.saveOrder(updated);
        return updated;
      }
      return o;
    ...
    setTables(prev => {
      ...
      return prev.map(t => {
        if (t.id === fromId) {
          const updated = { ...t, status: TableStatus.AVAILABLE, mergedWith: [] };
          if (isFirebaseEnabled) firestoreService.saveTable(updated);
          return updated;
        }
        if (t.id === toId) {
          const updated = { ...t, status: TableStatus.OCCUPIED, mergedWith: fromTable.mergedWith };
          if (isFirebaseEnabled) firestoreService.saveTable(updated);
          return updated;
        }
        return t;
  ...
  const addCustomer = (c: Customer) => {
    ...
    if (isFirebaseEnabled) firestoreService.saveCustomer(c);
  };

  const updateCustomer = (c: Customer) => {
    setCustomers(prev => prev.map(cus => cus.id === c.id ? c : cus));
    if (isFirebaseEnabled) firestoreService.saveCustomer(c);
  };

  const deleteCustomer = (id: string) => {
    setCustomers(prev => prev.filter(c => c.id !== id));
    if (isFirebaseEnabled) firestoreService.deleteCustomerFromDb(id);
  };
  ...
  const updateCustomerLoyalty = (customerId: string, pointsToAdd: number, incrementVisit: boolean) => {
    setCustomers(prev => prev.map(c => {
      if (c.id === customerId) {
        ...
        const updated = { ...c, points: newPoints, visitCount: newVisitCount, tier: calculateTier(newPoints) };
        if (isFirebaseEnabled) firestoreService.saveCustomer(updated);
        return updated;
      }
      return c;
  ...
  const redeemCouponForCustomer = (customerId: string, couponId: string): boolean => {
    ...
    const updated = { ...customer, points: newPoints, tier: calculateTier(newPoints), ownedCoupons: [...(customer.ownedCoupons || []), coupon.code] };
  ...
  const addCoupon = (c: Coupon) => {
    ...
    if (isFirebaseEnabled) firestoreService.saveCoupon(c);
  };

  const updateCoupon = (c: Coupon) => {
    setCoupons(prev => prev.map(coup => coup.id === c.id ? c : coup));
    if (isFirebaseEnabled) firestoreService.saveCoupon(c);
  };

  const deleteCoupon = (id: string) => {
    setCoupons(prev => prev.filter(c => c.id !== id));
    if (isFirebaseEnabled) firestoreService.deleteCouponFromDb(id);
  };
  ...
  const addUser = (u: User) => {
    ...
    if (isFirebaseEnabled) firestoreService.saveUser(u);
  };
  ...
  const deleteUser = (id: string) => {
    ...
    if (isFirebaseEnabled) firestoreService.deleteUserFromDb(id);
  };

  const addRole = (r: Role) => {
    setRoles(prev => [...prev, r]);
    if (isFirebaseEnabled) firestoreService.saveRole(r);
  };
  ...
  const deleteRole = (id: string) => {
    ...
    if (isFirebaseEnabled) firestoreService.deleteRoleFromDb(id);
  };
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      if (isFirebaseConfigured()) {
        ...
        try {
          // Initialize Database if empty (for demo)
          await firestoreService.initializeDatabase({
            menu: INITIAL_MENU, inventory: INITIAL_INVENTORY, tables: INITIAL_TABLES,
            customers: INITIAL_CUSTOMERS, settings: INITIAL_SETTINGS, bookings: INITIAL_BOOKINGS,
            users: INITIAL_USERS, roles: INITIAL_ROLES, zones: INITIAL_ZONES, categories: INITIAL_CATEGORIES, coupons: INITIAL_COUPONS
          });

          // 1. One-time loads for static/less dynamic data
          const [loadedMenu, loadedCats, loadedZones, loadedInventory, loadedCustomers, loadedCoupons, loadedBookings, loadedUsers, loadedRoles] = await Promise.all([
            firestoreService.loadMenu(),
            firestoreService.loadCategories(),
            firestoreService.loadZones(),
            firestoreService.loadInventory(),
            firestoreService.loadCustomers(),
            firestoreService.loadCoupons(),
            firestoreService.loadBookings(),
            firestoreService.loadUsers(),
            firestoreService.loadRoles()
          ]);
          ...
          const settings = await firestoreService.loadSettings();
          if (settings) setSettings(settings);

          // 2. Real-time Subscriptions for Dynamic Data (Orders & Tables)
          console.log("🔌 Activating Real-Time Listeners...");
          const unsubOrders = firestoreService.subscribeToOrders((updatedOrders) => {
            console.log("🔥 Real-Time Orders Update:", updatedOrders.length, "orders received");
            setOrders(updatedOrders);
          });

          const unsubTables = firestoreService.subscribeToTables((updatedTables) => {
            setTables(updatedTables);
          });

          // Check Admin Password Migration logic (One-time check on load)
          const adminUser = loadedUsers.find(u => u.username === 'admin');
          const mockAdmin = INITIAL_USERS.find(u => u.username === 'admin');
          if (adminUser && mockAdmin && adminUser.password !== mockAdmin.password && mockAdmin.password === '1234') {
            // If live admin pass is old (e.g. '123') and mock says '1234', update it.
            // Actually, logic: if loaded is '123' and we want '1234'.
            if (adminUser.password === '123') {
              console.log("Migrating Admin Password...");
              firestoreService.saveUser({ ...adminUser, password: '1234' });
            }
          } else if (!adminUser && mockAdmin) {
            // Creating admin if missing (re-init safety)
            firestoreService.saveUser(mockAdmin);
          }
