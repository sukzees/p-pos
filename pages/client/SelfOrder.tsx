...
import { ShoppingBag, X, Plus, Minus, ChefHat, CheckCircle2, ArrowLeft, MessageSquare, User, LogIn, Phone, Bell, Clock } from 'lucide-react';

const SelfOrder: React.FC = () => {
    const { tableId } = useParams<{ tableId: string }>();
    const navigate = useNavigate();
    const { menu, categories, tables, addOrder, settings, customers, addCustomer, toggleTableCall, orders } = useStore();
    ...
    const tableOrders = orders.filter(o => o.tableId === tableId && o.status !== OrderStatus.CANCELLED).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
        ...
        <div className="bg-gray-50 min-h-screen pb-24 relative">
                ...
                <div className="flex gap-2">
                    ...
                    {currentTable && (
                        <button
                            ...
                            className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${currentTable.isCallingStaff
            ...
            <div className="sticky top-[60px] z-10 bg-gray-50/95 backdrop-blur py-2 px-4 border-b overflow-x-auto flex gap-2 no-scrollbar">
                <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${activeCategory === 'All' ? 'bg-amber-600 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200'
                        }`}
                >
                    All Items
                </button>
                {categories.map(cat => (
                    <button
                        ...
                        className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${activeCategory === cat.id ? 'bg-amber-600 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200'
                            }`}
                    >
                        {cat.name}
                    </button>
                            ...
                            tableOrders.map((order) => (
                                <div key={order.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                                    <div className="bg-gray-50 px-4 py-3 border-b flex justify-between items-center">
                                        ...
                                        <div className={`px-2 py-1 rounded text-xs font-bold ${order.status === 'Served' ? 'bg-green-100 text-green-700' :
